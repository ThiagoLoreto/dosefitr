...
**Configuring the construct/compound separator (`label_sep`):**

By default, construct and compound names are joined with `":"` (e.g.
`EPHA1:KK135`). If your names already contain colons, choose a different
separator with `label_sep`:
...
The separator is stored as an attribute and propagated automatically through
the entire pipeline — you only need to set it once. The same parameter is
available in `batch_viability_analysis()`.
