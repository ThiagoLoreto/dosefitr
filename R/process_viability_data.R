...
process_viability_data <- function(data,
  ...
  detect_specific_pattern <- function(data) {
    ...
    if (header_start_col + 1 <= ncol(data)) {
      ...
      check_col_end <- min(header_start_col + 24, ncol(data))
      outer_break <- FALSE
      for (.check_row in data_start_row:check_row_end) {
        for (.check_col in (header_start_col + 1):check_col_end) {
          sample_cell <- data[.check_row, .check_col]
